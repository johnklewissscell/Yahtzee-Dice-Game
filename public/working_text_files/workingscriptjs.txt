...
function showWaitingOverlay(show) {
  ...
  const keeperDice = document.getElementById("keeper-dice");
  if (!overlay) return;

  overlay.style.display = show ? "flex" : "none";

  if (keeperDice) {
    keeperDice.style.visibility = show ? "hidden" : "visible";
  }
...
function handleMultiplayerMessage(message) {
  switch (message.type) {
    ...
    case "playerFinished":
      ...
      if (message.allPlayers) totalPlayers = message.allPlayers.length;

      const myName = sessionStorage.getItem("playerName");

      if (finishedPlayers[myName]) {
        showWaitingOverlay(true);
      }

      if (
        Object.keys(finishedPlayers).length === totalPlayers &&
        totalPlayers > 0
      ) {
...
window.addEventListener("load", () => {
  ...
  if (gameCode) {
    const gameRef = ref(rtdb, "games/" + gameCode);
...
function calculatePotentialScores() {
  ...
  if (yahtzeeScoreLocked && yahtzeeScoreValue === 0) {
    yahtzeeBonus = 0;
    document.getElementById("yahtzee-bonus-score").textContent = "0";
  } else if (
    yahtzeeScoreLocked &&
...
function displayFinalScores(scores) {
  ...
  const scoresBody = document.getElementById("final-scores-body");
  const keeperDice = document.getElementById("keeper-dice");

  if (!scoresContainer || !scoresBody) return;
  ...
  if (document.getElementById("mid-section"))
    document.getElementById("mid-section").style.display = "none";
  if (document.getElementById("scorecard"))
    document.getElementById("scorecard").style.display = "none";
  if (keeperDice) keeperDice.style.display = "none";

  scoresContainer.style.display = "block";

  const lobbyBtn = document.getElementById("return-lobby");
